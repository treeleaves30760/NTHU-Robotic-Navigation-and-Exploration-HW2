...
def cubic_spline(x, y, interval=2):
    ...
    b = [(y[i+1]-y[i])/h[i] - h[i]*m[i]/2 - h[i]
         * (m[i+1]-m[i])/6 for i in range(size-1)]
...
def cubic_spline_2d(path, interval=2):
    ...
    dx, ddx, dy, ddy = np.array(dx_list), np.array(
        ddx_list), np.array(dy_list), np.array(ddy_list)
    yaw_list = np.rad2deg(np.arctan2(dy, dx))
    curv_list = (ddy * dx - ddx * dy) / ((dx ** 2 + dy ** 2)**(3 / 2))
    path_smooth = [(x_list[i], y_list[i], yaw_list[i], curv_list[i])
...
if __name__ == "__main__":
    ...
    for i in range(len(path_smooth)-1):
        cv2.line(img, pos_int(path_smooth[i]), pos_int(
            path_smooth[i+1]), (1.0, 0.4, 0.4), 1)
